...
param(
    ...
    [string]$RemoteProject = "/home/msminipc/projects/sas",
...
$stageRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("sas-deploy-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    Invoke-NativeCommand `
        ...
        -FailureMessage "Could not download the addon locale files."

    $requiredFiles = @(
        (Join-Path $stageAddon "SimpleArsenalSwap.toc"),
        (Join-Path $stageAddon "SimpleArsenalSwap_TBC.toc"),
        (Join-Path $stageAddon "Core.lua"),
        (Join-Path $stageAddon "Swap.lua"),
        (Join-Path $stageAddon "Options.lua"),
    ...
    Write-Host ""
    Write-Host "Simple Arsenal Swap was updated successfully." -ForegroundColor Green
    Write-Host "In WoW, enter /reload and then /sas to configure it." -ForegroundColor Green
}
catch {
    $exitCode = 1
    Write-Host ""
